...
//     let breakdown = careers.map((career) => {
//       const rawScore = scores[career.key] || 0;
//       const percentage = ((rawScore / totalAnswers) * 100).toFixed(0);
...
//     const topCareer = breakdown[0]?.careerName;
...
// ✅ ADD THIS (MISSING FUNCTION - FIXES YOUR ERROR)
exports.getLatestQuiz = async (req, res) => {
  try {
    const latest = await QuizResult.findOne({ user: req.user._id }).sort({
      createdAt: -1,
    });
